...
names(original) 
original= original[c("Av_Lev_Aut","B2Au","B3Au","B4Au","B5Au", 
                       "B6Au","BUAu" , "LSTAu", "NDMIAu",
                       "NDVIAu","SAVIAu","Slope")] # remove unnecessary variables
names(original)
original$Av_Lev_Aut=as.factor(original$Av_Lev_Aut)
...
#Creating seperate dataframe for "Av_Lev_Win" features 
...
# Step 5 modeling
set.seed(849)
fit.xgb_train= train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                        NDVIAu+SAVIAu+Slope, 
                      data=train.data,
                      method = "xgbTree",
                      metric= "Accuracy",
...
set.seed(849)
fit.xgb_train2= train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                         NDVIAu+SAVIAu+Slope,  
                       data=train.data,
                       method = "xgbTree",
                       metric= "Accuracy",
...
summaryRes=fit.xgb_train2$results
head(summaryRes)
summary(summaryRes)
...
set.seed(849)
fit.xgb_train3= train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                         NDVIAu+SAVIAu+Slope,
                       data=train.data,
                       method = "xgbTree",
                       metric= "Accuracy",
...
ggplot(var_imp, aes(x = reorder(Var2, Importance), y = Importance)) + 
  ...
  ggtitle("Variable Importance from XGBoost Model Autumn") +
...
predictions_XGB_A = as.data.frame(predict(fit.xgb_train3, test.data, type = "prob"))

# predict class and then attach test class
predictions_XGB_A$predict = names(predictions_XGB_A)[1:2][apply(predictions_XGB_A[,1:2], 1, which.max)]
predictions_XGB_A$observed = test.data_labels
head(predictions_XGB_A)

# ROC PLOT
roc.Moderate = roc(ifelse(predictions_XGB_A$observed== "Moderate", "Moderate", "non-Moderate"), as.numeric(predictions_XGB_A$Moderate))
roc.UHealSen = roc(ifelse(predictions_XGB_A$observed=="UHealSen", "UHealSen", "non-UHealSen"), as.numeric(predictions_XGB_A$UHealSen))
roc.XGB_A = roc(ifelse(predictions_XGB_A$observed== "Moderate", "Moderate", "non-Moderate"), as.numeric(predictions_XGB_A$Moderate))
...
set.seed(849)
fit.xgbAll= train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                     NDVIAu+SAVIAu+Slope, 
                   data=original.n,
                   method = "xgbTree",
                   metric= "Accuracy",
...
B2Au = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B2_Au.tif" )
names(B2Au) = "B2Au"
B3Au = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B3_Au.tif" )
names(B3Au) = "B3Au"
B4Au = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B4_Au.tif")
names(B4Au) = "B4Au"
B5Au = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B5_Au.tif")
names(B5Au) = "B5Au"
B6Au = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B6_Au.tif")
names(B6Au) = "B6Au"
BUAu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/BU_Au.tif")
names(BUAu) = "BUAu"
DEM = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/DEM.tif")
LSTAu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/LST_Au.tif")
names(LSTAu) = "LSTAu"
NDBIAu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/NDBI_Au.tif")
names(NDBIAu) = "NDBIAu"
NDMIAu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/NDMI_Au.tif")
names(NDMIAu) = "NDMIAu"
NDVIAu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/NDVI_Au.tif")
names(NDVIAu) = "NDVIAu"
distoroad = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/RdDen.tif")
SAVIAu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/SAVI_Au.tif")
names(SAVIAu) = "SAVIAu"
...
Rasters= stack(B2Au,B3Au,B4Au,B5Au,B6Au,BUAu,LSTAu,NDMIAu,
                NDVIAu,SAVIAu,Slope)
plot(Rasters$LSTAu)
names(Rasters)
...
proj4string(r_ave_UHealSen)=CRS(projection(NDVIAu))

r_ave_Moderate = rasterFromXYZ(as.data.frame(x)[, c("x", "y", "Levels_Moderate")])
proj4string(r_ave_Moderate)=CRS(projection(NDVIAu))

# Plot Maps
spplot(r_ave_Moderate, main="Moderate XGB")
writeRaster(r_ave_Moderate,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Moderate_XGB_Aut.tif", format="GTiff", overwrite=TRUE) 

spplot(r_ave_UHealSen, main="UHealSen XGB")
writeRaster(r_ave_UHealSen,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_UHealSen_XGB_Aut.tif", format="GTiff", overwrite=TRUE) 
...
proj4string(r_ave)=CRS(projection(SAVIAu)) # set it to lat-long

# Export final prediction map as raster TIF ---------------------------
# write to a new geotiff file
spplot(r_ave)
writeRaster(r_ave,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Classification_Map XGB Autumn.tif", format="GTiff", overwrite=TRUE) 
...
set.seed(849)
fit.nb_def = train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                    NDVIAu+SAVIAu+Slope,  
                    data=train.data,
                    method = "nb",
                    metric= "Accuracy",
...
#We will use the train() function of the caret package with the "method" parameter "nb" wrapped from the e1071 package.
set.seed(849)
fit.nb = train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                  NDVIAu+SAVIAu+Slope,  
                data=train.data,
                method = "nb",
...
tune_gridNaive2 = expand.grid(fL= c(0.5) ,             # (Laplace Correction)
...
#We will use the train() function of the caret package with the "method" parameter "nb" wrapped from the e1071 package.
set.seed(849)
fit.nb2 = train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                   NDVIAu+SAVIAu+Slope, 
                 data=train.data,
                 method = "nb",
...
predictions_NB_A = as.data.frame(predict(fit.nb2, test.data, type = "prob"))

# predict class and then attach test class
predictions_NB_A$predict = names(predictions_NB_A)[1:2][apply(predictions_NB_A[,1:2], 1, which.max)]
predictions_NB_A$observed = test.data_labels
head(predictions_NB_A)


# 1 ROC curve, 
roc.UHealSen = roc(ifelse(predictions_NB_A$observed== "UHealSen", "UHealSen", "non-UHealSen"), as.numeric(predictions_NB_A$UHealSen))
roc.Moderate = roc(ifelse(predictions_NB_A$observed=="Moderate", "Moderate", "non-Moderate"), as.numeric(predictions_NB_A$Moderate))
roc.NB_A = roc(ifelse(predictions_NB_A$observed== "UHealSen", "UHealSen", "non-UHealSen"), as.numeric(predictions_NB_A$UHealSen))
...
original.n$PM=original$Av_Lev_Aut
set.seed(849)
fit.nbAll= train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                    NDVIAu+SAVIAu+Slope,
                  data=original.n,
                  method = "xgbTree",
                  metric= "Accuracy",
...
plot(X.nbAll, main="varImportance NB Autumn tunned")
...
writeRaster(r_ave_Moderate,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Moderate_NB_Aut.tif", format="GTiff", overwrite=TRUE) 

spplot(r_ave_UHealSen, main="UHealSen NB")
writeRaster(r_ave_UHealSen,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_UHealSen_NB_Aut.tif", format="GTiff", overwrite=TRUE) 
...
# write to a new geotiff file
spplot(r_ave)
writeRaster(r_ave,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Classification_Map NB Autumn.tif", format="GTiff", overwrite=TRUE) 
...
set.seed(1)
knn_grid = train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                   NDVIAu+SAVIAu+Slope, 
                 data=train.data,
                 method = "knn",
                 trControl = control,
...
#Fitted Parameter
set.seed(1)
param_grid = expand.grid(k=7)
knn_default = train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                      NDVIAu+SAVIAu+Slope,
                    data=train.data,
                    method = "knn",
                    trControl = control,
...
# the model is used to predict the test data. However, you should ask for type="prob" here
predictions_KNN_A = as.data.frame(predict(knn_default, test.data, type = "prob"))

# predict class and then attach test class
predictions_KNN_A$predict = names(predictions_KNN_A)[1:2][apply(predictions_KNN_A[,1:2], 1, which.max)]
predictions_KNN_A$observed = test.data_labels
head(predictions_KNN_A)


# 1 ROC curve, 
roc.UHealSen = roc(ifelse(predictions_KNN_A$observed== "UHealSen", "UHealSen", "non-UHealSen"), as.numeric(predictions_KNN_A$UHealSen))
roc.Moderate = roc(ifelse(predictions_KNN_A$observed=="Moderate", "Moderate", "non-Moderate"), as.numeric(predictions_KNN_A$Moderate))
roc.KNN_A = roc(ifelse(predictions_KNN_A$observed== "UHealSen", "UHealSen", "non-UHealSen"), as.numeric(predictions_KNN_A$UHealSen))

plot(roc.KNN_A)
# Plot the ROC for the three model
plot(roc.KNN_A, col = "red", lwd = 2, main = "ROC Plot for Autumn Season")
plot(roc.XGB_A, add = T, col = "green", lwd = 2)
plot(roc.NB_A, add = T, col = "blue", lwd = 2)
# Add AUC values
auc_KNN = auc(roc.KNN_A)
auc_XGB = auc(roc.XGB_A)
auc_NB = auc(roc.NB_A)
...
fit.KNNAll= train(PM~B2Au+ B3Au+ B4Au +B5Au+B6Au+ BUAu+ LSTAu+NDMIAu+
                     NDVIAu+SAVIAu+Slope, 
                   data=original.n,
                   method = "knn",
                   metric= "Accuracy",
...
writeRaster(r_ave_Moderate,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Moderate_KNN_Aut.tif", format="GTiff", overwrite=TRUE) 

spplot(r_ave_UHealSen, main="UHealSen KNN")
writeRaster(r_ave_UHealSen,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_UHealSen_KNN_Aut.tif", format="GTiff", overwrite=TRUE) 
...
# write to a new geotiff file
spplot(r_ave)
writeRaster(r_ave,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Classification_Map KNN Autumn.tif", format="GTiff", overwrite=TRUE) 
