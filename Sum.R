...
names(original) 
original = original[c("Av_Lev_Sum","B2Su","B3Su","B4Su","B5Su", 
                       "B6Su", "BUSu" , "LSTSu", "NDMISu",
                       "NDVISu","SAVISu","Slope")] # remove unnecessary variables
names(original)
original$Av_Lev_Sum=as.factor(original$Av_Lev_Sum)
...
number.perfect.splits = apply(X=original.n, MARGIN = 2, FUN = function(col){
  t= table(original$Av_Lev_Sum,col)
  sum(t == 0)})
...
# Step 5 modeling
set.seed(849)
fit.xgb_train= train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                        NDVISu+SAVISu+Slope,
                      data=train.data,
                      method = "xgbTree",
                      metric= "Accuracy",
...
set.seed(849)
fit.xgb_train2= train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+NDBISu+
                         NDVISu+SAVISu+Slope,
                       data=train.data,
                       method = "xgbTree",
                       metric= "Accuracy",
...
# Save it
write.csv(fit.xgb_train2$results,file = "fit.xgb_train_hyper_Sum.csv")#, sep = "",row.names = T)
...
tune_grid3 = expand.grid(nrounds = c(210),           # the max number of iterations INCREASE THE PROCESSING TIME COST
                          max_depth = c(22),           # depth of a tree EFFECTIVE OPTIMIZATION
...
set.seed(849)
fit.xgb_train3= train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                         NDVISu+SAVISu+Slope,
                       data=train.data,
                       method = "xgbTree",
                       metric= "Accuracy",
...
ggplot(var_imp, aes(x = reorder(Var2, Importance), y = Importance)) + 
  ...
  ggtitle("Variable Importance from XGBoost Model Summer") +
...
predictions_XGB_S = as.data.frame(predict(fit.xgb_train3, test.data, type = "prob"))

##  Since you have probabilities, use them to get the most-likely class.
# predict class and then attach test class
predictions_XGB_S$predict = names(predictions_XGB_S)[1:2][apply(predictions_XGB_S[,1:2], 1, which.max)]
predictions_XGB_S$observed = test.data_labels
head(predictions_XGB_S)

#Plot ROC 
roc.Good <- roc(ifelse(predictions_XGB_S$observed== "Good", "Good", "non-Good"), as.numeric(predictions_XGB_S$Good))
roc.Moderate <- roc(ifelse(predictions_XGB_S$observed=="Moderate", "Moderate", "non-Moderate"), as.numeric(predictions_XGB_S$Moderate))
roc.XGB_S <- roc(ifelse(predictions_XGB_S$observed== "Good", "Good", "non-Good"), as.numeric(predictions_XGB_S$Good))

plot(roc.XGB_S)
plot(roc.Good, col = "#FF9900", main="XGBoost best tune prediction ROC plot for Summer Season", xlim=c(0.64,0.1))
lines(roc.Moderate, col = "red")
...
original.n$PM=original$Av_Lev_Sum
set.seed(849)
fit.xgbAll= train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                     NDVISu+SAVISu+Slope, 
                   data=original.n,
                   method = "xgbTree",
                   metric= "Accuracy",
...
# Plot graph
# 2. Create the plot
plot(X.xgbAll,main="varImportance All Summer XB" )
# 3. Close the file
dev.off()
...
B2Su = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B2_Su.tif" )
names(B2Su) = "B2Su"
B3Su = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B3_Su.tif" )
names(B3Su) = "B3Su"
B4Su = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B4_Su.tif")
names(B4Su) = "B4Su"
B5Su = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B5_Su.tif")
names(B5Su) = "B5Su"
B6Su = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/B6_Su.tif")
names(B6Su) = "B6Su"
BUSu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/BU_Su.tif")
names(BUSu) = "BUSu"
DEM = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/DEM.tif")
LSTSu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/LST_Su.tif")
names(LSTSu) = "LSTSu"
NDBISu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/NDBI_Su.tif")
names(NDBISu) = "NDBISu"
NDMISu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/NDMI_Su.tif")
names(NDMISu) = "NDMISu"
NDBISu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/NDBI_Su.tif")
names(NDBISu) = "NDBISu"
NDVISu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/NDVI_Su.tif")
names(NDVISu) = "NDVISu"
distoroad = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/RdDen.tif")
SAVISu = raster("C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/SAVI_Su.tif")
names(SAVISu) = "SAVISu"
...
Rasters= stack(B2Su,B3Su,B4Su,B5Su,B6Su,BUSu,LSTSu,NDMISu,
                NDVISu,SAVISu,Slope)
plot(Rasters$BUSu)
names(Rasters)
...
proj4string(r_ave_Good)=CRS(projection(NDVISu))

r_ave_Moderate = rasterFromXYZ(as.data.frame(x)[, c("x", "y", "Levels_Moderate")])
proj4string(r_ave_Moderate)=CRS(projection(NDVISu))

# Plot Maps
spplot(r_ave_Moderate, main="Moderate XGB")
writeRaster(r_ave_Moderate,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Moderate_XGB_Sum.tif", format="GTiff", overwrite=TRUE) 

spplot(r_ave_Good, main="GOOD XGB")
writeRaster(r_ave_Good,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Good_XGB_Sum.tif", format="GTiff", overwrite=TRUE) 
...
proj4string(r_ave)=CRS(projection(SAVISu)) # set it to lat-long

# Export final prediction map as raster TIF ---------------------------
# write to a new geotiff file
writeRaster(r_ave,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Classification_Map XGB Summer.tif", format="GTiff", overwrite=TRUE) 
...
set.seed(849)
fit.nb_def = train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                      NDVISu+SAVISu+Slope,  
                    data=train.data,
                    method = "nb",
                    metric= "Accuracy",
...
set.seed(849)
fit.nb = train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                  NDVISu+SAVISu+Slope, 
                data=train.data,
                method = "nb",
...
#We will use the train() function of the caret package with the "method" parameter "nb" wrapped from the e1071 package.
set.seed(849)
fit.nb2 = train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                   NDVISu+SAVISu+Slope, 
                 data=train.data,
                 method = "nb",
...
ggplot(var_imp, aes(x = reorder(Var2, Importance), y = Importance)) + 
  geom_bar(stat = "identity") + 
  xlab("Variable") + ylab("Importance") + 
  ggtitle("Variable Importance from NB Model Summer") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
# the model is used to predict the test data. However, you should ask for type="prob" here
predictions_NB_S = as.data.frame(predict(fit.nb2, test.data, type = "prob"))

##  Since you have probabilities, use them to get the most-likely class.
# predict class and then attach test class
predictions_NB_S$predict = names(predictions_NB_S)[1:2][apply(predictions_NB_S[,1:2], 1, which.max)]
predictions_NB_S$observed = test.data_labels
head(predictions_NB_S)


# 1 ROC curve,  UHeal,  UHealSen vs non  UHeal non  UHealSen
roc.Good = roc(ifelse(predictions_NB_S$observed== "Good", "Good", "non-Good"), as.numeric(predictions_NB_S$Good))
roc.Moderate = roc(ifelse(predictions_NB_S$observed=="Moderate", "Moderate", "non-Moderate"), as.numeric(predictions_NB_S$Moderate))
roc.NB_S = roc(ifelse(predictions_NB_S$observed== "Good", "Good", "non-Good"), as.numeric(predictions_NB_S$Good))
...
original.n$PM=original$Av_Lev_Sum
set.seed(849)
fit.nbAll= train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                     NDVISu+SAVISu+Slope, 
                   data=original.n,
                   method = "xgbTree",
                   metric= "Accuracy",
...
plot(X.nbAll, main="varImportance NB Summer tunned")

# Plot graph
plot(X.nbAll,main="varImportance All Summer NB" )
# 3. Close the file
dev.off()
...
writeRaster(r_ave_Moderate,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Moderate_NB_Sum.tif", format="GTiff", overwrite=TRUE) 

spplot(r_ave_Good, main="GOOD NB")
writeRaster(r_ave_Good,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Good_NB_Sum.tif", format="GTiff", overwrite=TRUE) 
...
# Export final prediction map as raster TIF ---------------------------
# write to a new geotiff file
writeRaster(r_ave,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Classification_Map NB Summer.tif", format="GTiff", overwrite=TRUE) 
...
set.seed(1)
knn_grid = train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                   NDVISu+SAVISu+Slope, 
                 data=train.data,
                 method = "knn",
                 trControl = control,
                 tuneGrid = param_grid)
plot(knn_grid, main="KNN with different K values For Summer")
...
ggplot(var_imp, aes(x = reorder(Var2, Importance), y = Importance)) + 
  geom_bar(stat = "identity") + 
  xlab("Variable") + ylab("Importance") + 
  ggtitle("Variable Importance from KNN Model Summer") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
# Evaluate the model
...
#Fitted Parameter
set.seed(1)
param_grid = expand.grid(k=5)
knn_default = train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                      NDVISu+SAVISu+Slope, 
                    data=train.data,
                    method = "knn",
                    trControl = control,
...
predictions_KNN_S = as.data.frame(predict(knn_default, test.data, type = "prob"))

# predict class and then attach test class
predictions_KNN_S$predict = names(predictions_KNN_S)[1:2][apply(predictions_KNN_S[,1:2], 1, which.max)]
predictions_KNN_S$observed = test.data_labels
head(predictions_KNN_S)

#Plot ROC
roc.Good = roc(ifelse(predictions_KNN_S$observed== "Good", "Good", "non-Good"), as.numeric(predictions_KNN_S$Good))
roc.Moderate = roc(ifelse(predictions_KNN_S$observed=="Moderate", "Moderate", "non-Moderate"), as.numeric(predictions_KNN_S$Moderate))
roc.KNN_S = roc(ifelse(predictions_KNN_S$observed== "Good", "Good", "non-Good"), as.numeric(predictions_KNN_S$Good))

plot(roc.KNN_S)
plot(roc.Good, col = "#FF9900", main="NB best tune prediction ROC plot for Summer Season", xlim=c(0.64,0.1))
lines(roc.Moderate, col = "red")

# Plot the ROC for the three model
plot(roc.KNN_S, col = "red", lwd = 2, main = "ROC Plot for Summer Season")
plot(roc.XGB_S, add = T, col = "green", lwd = 2)
plot(roc.NB_S, add = T, col = "blue", lwd = 2)
# Add AUC values
auc_KNN =auc(roc.KNN_S)
auc_XGB = auc(roc.XGB_S)
auc_NB =auc(roc.NB_S)
...
fit.KNNAll= train(PM~B2Su+ B3Su+ B4Su +B5Su+B6Su+BUSu+LSTSu+NDMISu+
                     NDVISu+SAVISu+Slope, 
                   data=original.n,
                   method = "knn",
                   metric= "Accuracy",
...
x=SpatialPointsDataFrame(as.data.frame(Rasters.df)[, c("x", "y")], data = Rasters.df)
r_ave_Good = rasterFromXYZ(as.data.frame(x)[, c("x", "y", "Levels_Good")])
proj4string(r_ave_Good)=CRS(projection(SAVISu))

r_ave_Moderate = rasterFromXYZ(as.data.frame(x)[, c("x", "y", "Levels_Moderate")])
proj4string(r_ave_Moderate)=CRS(projection(SAVISu))


# Plot Maps
spplot(r_ave_Good, main="Good KNN")
writeRaster(r_ave_Good,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Good_KNN_Sum.tif", format="GTiff", overwrite=TRUE) 

spplot(r_ave_Moderate, main="Moderate KNN")
writeRaster(r_ave_Moderate,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Probability_Map_Moderate_KNN_Sum.tif", format="GTiff", overwrite=TRUE)
...
# Export final prediction map as raster TIF ---------------------------
# write to a new geotiff file
writeRaster(r_ave,filename="C:/Users/user/Desktop/TROPHEE/Scientific training/Remote sensing method/Landsat_Images/Classification_Map KNN Summer.tif", format="GTiff", overwrite=TRUE) 
